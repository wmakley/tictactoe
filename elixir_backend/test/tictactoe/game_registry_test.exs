defmodule Tictactoe.GameRegistryTest do
  ...
  test "lookup_or_start_game adds games to the registry or returns them if they exist", %{
    ...
    {:ok, "dupe-test", pid} = GameRegistry.lookup_or_start_game(registry, "dupe-test")

    {:ok, "dupe-test", pid2} = GameRegistry.lookup_or_start_game(registry, "dupe-test")
  ...
  test "games are removed from the registry if they crash", %{
    ...
    {:ok, "will-crash", pid} = GameRegistry.lookup_or_start_game(registry, "will-crash")
  ...
  test "registry stays up if games crash", %{registry: registry} do
    # start two games
    {:ok, "test", pid} = GameRegistry.lookup_or_start_game(registry, "test")
    {:ok, "test2", pid2} = GameRegistry.lookup_or_start_game(registry, "test2")
    ...
    {:ok, "test2", pid3} = GameRegistry.lookup_or_start_game(registry, "test2")
    assert pid3 == pid2
  ...
  test "all games are killed if registry crashes", %{registry: registry} do
    # start two games
    {:ok, "game1", pid} = GameRegistry.lookup_or_start_game(registry, "game1")
    {:ok, "game2", pid2} = GameRegistry.lookup_or_start_game(registry, "game2")
